...
::: {#des-no.lineal.1 .callout-tip icon="false"}
## 📝 Desafío 1
...
::: {#des-no.lineal.2 .callout-tip icon="false"}
## 📝 Desafío 2
Se anima a describir algorítmicamente cuál fue el método que utilizó en el [Desafío 1](#des-no.lineal.1){.quarto-xref}.
:::
...
El @thm-condicion.punto.fijo nos dice que el método de punto fijo con una función $g$ converge a un $p$ tal que $p = g(p)$. Entonces,
...
Sí, pues cumple las hipótesis del @thm-existe.punto.fijo, ya que
$$
...
**Demostración del teorema del punto fijo (@thm-punto.fijo)**

Si $p_0 ∈ [p − δ,\ p + δ]$, entonces $|p - p_0| ≤ δ$. Ahora bien,
por el teorema del valor medio (@thm-valor.medio), existe $z_1$ entre $p$ y $p_0$ tal que
$$
...
Si $p$ es un número tal que $f(p) = 0$ se dice que $p$ es un *cero* de $f$ o que $p$ es una raíz de -@eq-f.de.x.igual.a.cero.

Antes de ver el método, recordamos el teorema del valor intermedio (@thm-valor.intermedio), que es la base que da la idea al método de bisección.
...
Recordando el Teorema del Punto Fijo (@thm-punto.fijo.v2) hemos deducido el siguiente teorema:
...
Probemos *iterar a mano*. Para ello, ejecute la siguiente celda 1 vez:
```{pyodide-python}
...
Luego, ejecute repetidamente esta celda:
```{pyodide-python}
...
Observando la @fig-curvas.de.nivel (curvas de nivel) tomemos como una aproximación inicial de una de las soluciones el punto  $\mathbf{p}_0 = [ 0, 1]^T$. Luego, iteremos sucesivamente la función $G$ haciendo $\mathbf{p}_{n}=G(\mathbf{p}_{n-1})$ como muestra el siguiente código:
