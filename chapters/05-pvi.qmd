...
2. Determine la cantidad de pasos $L$ y el número de evaluaciones de $f$ que fueron necesarios, en cada método, para obtener $y(2)$ con:
    ...
    (i) Tres decimales correctos.
    (ii) Seis decimales correctos.
    (iii) Diez decimales correctos.
:::
...
:::: {.callout-tip icon="false" collapse="true"}
## 📝 Ejercicio Adicional 2 (Modelo predador-presa de Lotka-Volterra)
...
preguntas:
    ...
    (i) ¿Cuál de las dos especies puede sobrevivir sin la existencia de la otra?
    (ii) ¿Qué especie se extingue si la otra no existe?

2. Graficar la evolución de ambas poblaciones considerando que la población inicial de presas es 1600 y la de predadores es 800, y teniendo en cuenta que el tiempo $t$ se mide en meses. Describir el fenómeno representado.
...
![Péndulo simple](images/pvi_ejemplo_pendulo_simple.png){fig-align="center" width="50%"}
...
1. $\phi(0) = 0.1,\quad \phi'(0) = 0$
2. $\phi(0) = 0.7,\quad \phi'(0) = 0$
3. $\phi(0) = 3.0,\quad \phi'(0) = 0$
4. $\phi(0) = 3.5,\quad \phi'(0) = 0$
5. $\phi(0) = 0,\quad \phi'(0) = 1$
6. $\phi(0) = 0,\quad \phi'(0) = 1.99$
7. $\phi(0) = 0,\quad \phi'(0) = 2$
8. $\phi(0) = 0,\quad \phi'(0) = 2.01$
:::
...
![Péndulo de Foucault](images/pvi_pendulo_foucault.png){fig-align="center" height=300}

1. Obtenga una aproximación de la posición del péndulo a tiempo $t = 70$ con un error absoluto menor a $10^{-8}$.

2. ¿Cuántas veces vuelve a pasar por la posición inicial (aproximadamente) en el período de tiempo $0 < t \leq 70$? También se desea saber en qué instantes aproximados esto ocurre (puede hacerlo a partir de un gráfico).

3. Obsérvese que el péndulo describe un gráfico similar a un flor ¿Cuántos pétalos tiene la misma para $t \in [0, 70]$?
