...
* `f=f(t,y)`: Es una función que recibe dos argumentos, `t` es un escalar, `y` es un vector *columna* de `n` componentes; y retorna un vector columna de `n` componentes, siendo `n=length(y0)`. Por ejemplo: `f = @(t,y) [-t*y(2) ; t*(y(1)-y(2))]`;
